...
public class LevelCreator {
	...
	public static List<Block> getLevelTwoBlocks() {
		...
		for (int i = 0; i < Settings.LEVEL2_COLS; i++) {
			for(int j = 0; j < Settings.LEVEL2_ROWS; j++) {
				// Spawn new block, then lower the point of origin to the next row

				blocks.add(new Block(new Point2D(originX, originY), new Vector2D(Settings.BLOCK_WIDTH, Settings.BLOCK_HEIGHT), Color.FIREBRICK, 5));
				originY -= Settings.BLOCK_SPACING + Settings.BLOCK_HEIGHT;
